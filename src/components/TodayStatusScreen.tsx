...
export const TodayStatusScreen: React.FC<TodayStatusScreenProps> = ({
  ...
  // Risk Calculations
  const riskScore = data.riskScoreInternal || 74;
  const isHighRisk = riskScore >= 70;
  const isModerateRisk = riskScore >= 35 && riskScore < 70;

  const riskColor = isHighRisk ? '#E11D48' : isModerateRisk ? '#D97706' : '#059669';
  const riskBgColor = isHighRisk ? 'bg-rose-50 border-rose-200' : isModerateRisk ? 'bg-amber-50 border-amber-200' : 'bg-emerald-50 border-emerald-200';
  const riskBadgeBg = isHighRisk ? 'bg-rose-100 text-rose-900 border-rose-200' : isModerateRisk ? 'bg-amber-100 text-amber-950 border-amber-200' : 'bg-emerald-100 text-emerald-950 border-emerald-200';

  // Confidence Calculations
  const confidenceScore = data.confidenceScoreInternal || 67;
  const confidenceColor = confidenceScore >= 75 ? '#059669' : confidenceScore >= 45 ? '#0284C7' : '#64748B';
  ...
  const riskOffset = circumferenceOuter - (riskScore / 100) * circumferenceOuter;
          ...
          <div className="relative w-44 h-44 flex items-center justify-center shrink-0 my-1">
            <svg className="w-full h-full -rotate-90 transform" viewBox="0 0 160 160">
              ...
              <circle
                ...
                onClick={() => setShowRiskModal(true)}
              >
                <title>Toca para ver el detalle de Riesgo ({riskScore}%)</title>
              </circle>
              ...
              <circle
                ...
                onClick={() => setShowConfidenceModal(true)}
              >
                <title>Toca para ver el cálculo de Confianza ({confidenceScore}%)</title>
              </circle>
            </svg>
