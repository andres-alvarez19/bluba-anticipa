...
export const ExpressReportScreen: React.FC<ExpressReportScreenProps> = ({
  ...
  const [captureMethod, setCaptureMethod] = useState<SchoolCaptureMethod>('TEXT');
  ...
  const [regulationState, setRegulationState] = useState<SchoolRegulationState | null>('Estable con apoyo');
  const [alertLevel, setAlertLevel] = useState<SchoolAlertLevel | null>('Habitual');
  const [observedBehaviors, setObservedBehaviors] = useState<string[]>(['Mayor necesidad de apoyo']);
  const [hadUnusualChange, setHadUnusualChange] = useState<SchoolRoutineChangeAnswer>('No');
  const [unusualChangeCategories, setUnusualChangeCategories] = useState<string[]>([]);
