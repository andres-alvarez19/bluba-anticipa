  ...
  'child-demo-1': [
    {
      ...
      tipsForCaregiver: 'Estrategia de catálogo seleccionada previamente para contextos de sobrecarga sensorial.'
    },
    {
      id: 'rec-mateo-2',
...
export const RecommendationsScreen: React.FC<RecommendationsScreenProps> = ({
  ...
  const childKey = childData.id in CHILD_RECOMMENDATIONS_MAP ? childData.id : 'child-demo-1';
  const childRecommendations = CHILD_RECOMMENDATIONS_MAP[childKey] || CHILD_RECOMMENDATIONS_MAP['child-demo-1'];
  ...
  // El estilo sigue el nivel recibido; esta vista no clasifica el score.
  const isHighRisk = childData.riskLevel === 'ELEVATED';
  const isModerateRisk = childData.riskLevel === 'MODERATE';
